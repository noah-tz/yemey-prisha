...
- **Extended by default** — all features enabled (reminders, API, Google login, password recovery)
- E2E mode (opt-in) — only password can decrypt, maximum privacy
...
| Mode | Default? | API/MCP | Reminders | Google Login | Password Recovery | Who can decrypt |
|------|:--------:|:-------:|:---------:|:------------:|:-----------------:|----------------|
| **🔔 Extended** | ✅ Yes | ✅ Works | ✅ Works | ✅ Works | ✅ Works | User + server (for automated processing) |
| **🔒 E2E** | ❌ Opt-in | ❌ Blocked | ❌ Disabled | ❌ Blocked | ❌ Disabled | Only the user (password-derived key) |

- **New users start in Extended mode** — all features work immediately (reminders, API, Google login, password recovery).
- **E2E mode** is opt-in for users who want maximum privacy. Requires a password to be set first.
- **Users can switch between modes at any time** from Settings.
