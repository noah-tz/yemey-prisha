...
function register(email, password) {
  ...
  // Extended mode by default: store enc_key_encrypted on registration
  // Users can opt-in to E2E mode later from settings (which deletes enc_key_encrypted)

  const encKeyWrapped = cryptoService.wrapKeyForStorage(encKey);

  try {
    const user = userRepository.create(email, hash, encSalt, encKeyWrapped);
