...
var Settings = (function() {
  ...
  function loadPasswordSection() {
    Api.get('/api/settings/has-password').then(function(data) {
      var descEl = document.getElementById('password-section-desc');
      var container = document.getElementById('password-form-container');
      var hasPassword = data.has_password;

      descEl.textContent = hasPassword ? I18n.t('settings_password_change_desc') : I18n.t('settings_password_set_desc');

      var html = '';
      if (hasPassword) {
        html += '<div class="form-group"><label>' + I18n.t('settings_password_current') + '</label><div class="password-wrapper"><input type="password" id="current-password"><button type="button" class="password-toggle" tabindex="-1">👁</button></div></div>';
      }
      html += '<div class="form-group"><label>' + I18n.t('settings_password_new') + '</label><div class="password-wrapper"><input type="password" id="new-password"><button type="button" class="password-toggle" tabindex="-1">👁</button></div></div>';
      html += '<div class="form-group"><label>' + I18n.t('settings_password_confirm') + '</label><div class="password-wrapper"><input type="password" id="confirm-new-password"><button type="button" class="password-toggle" tabindex="-1">👁</button></div></div>';
      html += '<button class="btn btn-primary" id="save-password-btn">' + I18n.t('settings_password_save') + '</button>';
      ...
      document.getElementById('save-password-btn').addEventListener('click', function() {
        ...
        var currentPw = hasPassword ? document.getElementById('current-password').value : '';
        var newPw = document.getElementById('new-password').value;
        var confirmPw = document.getElementById('confirm-new-password').value;

        if (!newPw || newPw.length < 6) {
          msgEl.className = 'error-message';
          msgEl.textContent = I18n.t('error_password_short');
          return;
        }
        if (newPw !== confirmPw) {
          msgEl.className = 'error-message';
          msgEl.textContent = I18n.t('error_passwords_mismatch');
          return;
        }

        var payload = { newPassword: newPw };
        if (hasPassword) payload.currentPassword = currentPw;

        Api.put('/api/settings/password', payload)
          .then(function() {
            msgEl.className = 'success-message';
            msgEl.textContent = I18n.t('settings_password_saved');
            setTimeout(function() { msgEl.textContent = ''; }, 3000);
            loadPasswordSection(); // Refresh
          })
          .catch(function(err) {
            msgEl.className = 'error-message';
            msgEl.textContent = err.message || I18n.t('error_generic');
    ...
    Api.get('/api/settings/encryption-mode')
      .then(function(data) {
        ...
        if (data.mode === 'e2e') {
          ...
          actionsEl.innerHTML = '<p style="font-size:0.8rem; color:var(--color-text-secondary); margin-bottom:0.75rem; line-height:1.6; background:var(--color-bg-secondary, #f8f9fa); padding:0.6rem; border-radius:6px;">ℹ️ ' + I18n.t('enc_tooltip_e2e') + '</p>' +
            '<button class="btn btn-primary" id="enable-extended-btn">' + I18n.t('enc_enable_extended') + '</button>';
          ...
          actionsEl.innerHTML = '<p style="font-size:0.8rem; color:var(--color-text-secondary); margin-bottom:0.75rem; line-height:1.6; background:var(--color-bg-secondary, #f8f9fa); padding:0.6rem; border-radius:6px;">ℹ️ ' + I18n.t('enc_tooltip_extended') + '</p>' +
            '<button class="btn btn-secondary" id="disable-extended-btn">' + I18n.t('enc_disable_extended') + '</button>';
          
          document.getElementById('disable-extended-btn').addEventListener('click', function() {
            // Check if user has a password before allowing E2E switch
            Api.get('/api/settings/has-password').then(function(pwData) {
              if (!pwData.has_password) {
                alert(I18n.t('settings_password_required_for_e2e'));
                return;
              }
