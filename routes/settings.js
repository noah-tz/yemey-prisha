 ...
 * Body: { currentPassword (optional if no password set), newPassword }
 */
router.put('/password', (req, res) => {
  try {
    ...
    const cryptoService = require('../services/crypto');
    const user = db.prepare('SELECT id, password_hash, enc_salt, enc_key_encrypted FROM users WHERE id = ?').get(req.userId);
    if (!user) return res.status(404).json({ error: 'User not found' });
    ...
    // Generate new enc_salt and derive key
    const encSalt = cryptoService.generateSalt();
    const encKey = cryptoService.deriveKey(newPassword, encSalt);

    // If user has data, re-encrypt with new key
    if (user.enc_key_encrypted) {
      const { loadUserData, saveUserData } = require('../services/userDataService');
      const oldKey = cryptoService.unwrapKeyFromStorage(user.enc_key_encrypted);
      const data = loadUserData(req.userId, oldKey);
      const newWrapped = cryptoService.wrapKeyForStorage(encKey);
      db.prepare('UPDATE users SET password_hash = ?, enc_salt = ?, enc_key_encrypted = ? WHERE id = ?')
        .run(newHash, encSalt, newWrapped, req.userId);
      saveUserData(req.userId, data, encKey);
    } else {
      // No enc_key_encrypted (shouldn't happen with new default, but handle)
      const newWrapped = cryptoService.wrapKeyForStorage(encKey);
      db.prepare('UPDATE users SET password_hash = ?, enc_salt = ?, enc_key_encrypted = ? WHERE id = ?')
        .run(newHash, encSalt, newWrapped, req.userId);
    }

    // Update session encKey
    req.session.encKey = encKey.toString('hex');

    return res.json({ message: 'password_saved', has_password: true });
  } catch (err) {
    return res.status(500).json({ error: 'Internal server error' });
  }
});
...
/**
 * GET /api/settings/has-password
 * Check if current user has a password set.
 */
router.get('/has-password', (req, res) => {
  const user = db.prepare('SELECT password_hash FROM users WHERE id = ?').get(req.userId);
  const hasPassword = user && user.password_hash && user.password_hash.length > 0;
  return res.json({ has_password: hasPassword });
});
